...
class Grid:
    ...
    def __str__(self): #surcharge de print
        s=""
...
class Game():  
    ...
    def __init__(self, size, joueur1, joueur2, joueur1_to_play): 
        ...
        if not joueur1_to_play:
           self._indice_courant=1
        
        try:
            self._Plateau=Grid(size)  
            self._Personnage=(size//2,size//2)
        except EvenSize: 
            print("La taille entrée en paramètre de Game est paire")
            print("Si vous choisissez de jouer à partir d'une grille "\
            + "aléatoire, la taille de celle-ci sera incrémentée d'une unité")
            print("Si vous choisissez d'importer la grille depuis un fichier"\
             + ", sa taille sera réinitialisée à celle présente dans le "\
             + "fichier")
            self._Plateau=Grid(size+1)
            self._Personnage=((size+1)//2,(size+1)//2)
        #PS : C'est ici que l'on traite l'exception car on a envie que le 
        #personnage que l'on va créer maintenant soit au milieu de la grille    
    ...
    #Affiche un message demandant au joueur de choisir une direction
        ...
        print("1    2    3") 
        print("")
        print(self._Joueurs[self._indice_courant].nom
    ...
    #de la direction choisie par l'utilisateur
        ...
        self._Joueurs[self._indice_courant]+=self._Plateau[self._Personnage] 
    ...
    def Result(self):   
        for joueur in self._Joueurs:
            print("Resultat de " + joueur.nom + " : " + str(joueur.score))
    ...
    #sont à égalité
        ...
        #self._indice_courant va être modifié lors des appels récursifs 
        ...
        if self.EndGame():
            ...
            if is_max:
               ...
               for direction in directions.keys(): #coup envisageable 
                   try:
    ...
    def PlayOnePlayer(self,profondeur):   #Humain contre I.A. 
        ...
        self.Show() 
        while not self.EndGame():
            if self._indice_courant==0:       #à l'humain de jouer
               self.AskPlayer()
               while True:
                   try:
                       direction = input()
    ...
    def SeeAGame(self,profondeur):        #I.A. contre I.A. (spectateur)
        ...
        while not self.EndGame():
             ...
             direction = self.MinMax(self._indice_courant,profondeur,True)[1]
             self.PlayStroke(direction)
             self.Show()
        self.Result()
